(*
   Copyright 2023 Microsoft Research

   Licensed under the Apache License, Version 2.0 (the "License");
   you may not use this file except in compliance with the License.
   You may obtain a copy of the License at

       http://www.apache.org/licenses/LICENSE-2.0

   Unless required by applicable law or agreed to in writing, software
   distributed under the License is distributed on an "AS IS" BASIS,
   WITHOUT WARRANTIES OR CONDITIONS OF ANY KIND, either express or implied.
   See the License for the specific language governing permissions and
   limitations under the License.
*)

module QuickSort.Task.NuPool

open Pulse.Lib.Pervasives
module A = Pulse.Lib.Array
module R = Pulse.Lib.Reference
module SZ = FStar.SizeT

open Pulse.Lib.InvList

open Pulse.Lib.Codeable
module P0 = NuPool
module T = NuPool.Code
open NuPool.Code
module TC = FStar.Tactics.Typeclasses
open NuPool.Autocode
open Quicksort.Base
open Pulse.Lib.Pledge

let quicksort_post a lo hi s0 lb rb : vprop =
  exists* s. A.pts_to_range a lo hi s ** pure (pure_post_quicksort a lo hi lb rb s0 s)

instance small_class_pts_to_range (t:Type0) (a:A.array t) (lo hi : erased nat) (s : erased (Seq.seq t))
  : Pulse.Class.Small.small (A.pts_to_range a lo hi s) = {
  pf = ();
}

```pulse
fn rec t_quicksort
  (p : T.pool)
  (#f : perm)
  (a : A.array int)
  (lo : nat) (hi : (hi:nat{lo <= hi}))
  (#lb #rb : erased int)
  (#s0 : erased (Seq.seq int))
  requires
    alive_proxy f p **
    A.pts_to_range a lo hi s0 **
    pure (pure_pre_quicksort a lo hi lb rb s0)
  ensures
    pledge (pool_inames p) (done_proxy p) (
      alive_proxy f p **
      quicksort_post a lo hi s0 lb rb
    )
{
  if (lo < hi - 1)
  {
    let r = partition_wrapper a lo hi lb rb;
    let pivot = r._3;
    with s1. assert (A.pts_to_range a lo r._1 s1);
    with s2. assert (A.pts_to_range a r._1 r._2 s2);
    with s3. assert (A.pts_to_range a r._2 hi s3);

    share_alive_proxy p;

    NuPool.Code.recover_alive p #(f /. 2.0R);

    (* We have to give the pre otherwise we run into a "Variable not found", but...
    the codes are autogenerated by the typeclass!. *)
    T.spawn_ p #(f /. 2.0R)
      #(alive_proxy (f /. 2.0R) p ** A.pts_to_range a lo r._1 s1 ** pure (pure_pre_quicksort a lo r._1 lb pivot s1))
      (fun () -> t_quicksort p #(f /. 2.0R) a lo r._1 #lb #pivot);

    pledge_sub_inv emp_inames (pool_inames p) _ _;
    strengthen_done_proxy_pledge p _;

    NuPool.Code.stash_alive p #(f /. 2.0R);

    t_quicksort p #(f /. 2.0R) a r._2 hi #pivot #rb;

    return_pledge (done_proxy p) (A.pts_to_range a r._1 r._2 s2);
    pledge_sub_inv emp_inames (pool_inames p) _ _;
    squash_pledge _ _ _;
    join_pledge _ _;
    join_pledge _ _;

    ghost fn rewrite_pf ()
      // NB: These two vprops have to be in exactly this shape, as the Pulse checker
      // will not commute or in anyway modify each side of the pledge. The function
      // above must also be in this exact shape. To obtain the shape, I just manually looked
      // at the context. Automation should likely help here.
      requires
        (alive_proxy (f /. 2.0R) p ** quicksort_post a lo r._1 s1 lb pivot) **
        A.pts_to_range a r._1 r._2 s2 **
        (alive_proxy (f /. 2.0R) p ** quicksort_post a r._2 hi s3 pivot rb)
      ensures
        alive_proxy f p **
        quicksort_post a lo hi s0 lb rb
    {
      (* Functional correctness *)

      unfold quicksort_post;
      unfold quicksort_post;

      quicksort_proof a lo r._1 r._2 hi lb rb pivot #s0 s1 s2 s3;

      fold (quicksort_post a lo hi s0 lb rb);

      (* Permission accounting *)
      T.gather_alive_proxy p;
    };
    rewrite_pledge _ _ rewrite_pf;

    ()
  } else {
    return_pledge (done_proxy p) (
      alive_proxy f p **
      (exists* s. A.pts_to_range a lo hi s ** pure (pure_post_quicksort a lo hi lb rb s0 s))
    );
    pledge_sub_inv emp_inames (pool_inames p) _ _;
  }
}
```

```pulse
fn rec quicksort
  (nthr : pos)
  (a : A.array int)
  (lo : nat) (hi : (hi:nat{lo <= hi}))
  (#lb #rb : erased int)
  (#s0 : erased (Seq.seq int))
  requires
    A.pts_to_range a lo hi s0 **
    pure (pure_pre_quicksort a lo hi lb rb s0)
  ensures
    quicksort_post a lo hi s0 lb rb
{
  let p = T.setup_pool #small_code nthr;
  T.share_alive p _;
  NuPool.Code.stash_alive p;
  t_quicksort p a lo hi #lb #rb;
  weaken_done_proxy_pledge p _;
  T.await_pool p _;
  NuPool.Code.recover_alive p;
  T.gather_alive p _;
  T.teardown_pool p;
  drop_ (T.pool_done p);
}
```
