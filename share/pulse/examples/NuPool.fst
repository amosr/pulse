...
fn add_one_state_pred (#code:vcode)
  ...
  requires task_thunk_typing code t
        ** state_pred t.pre t.post t.h
        ** all_state_pred code v_runnable
  ensures  all_state_pred code (t :: v_runnable)
{
  fold (all_state_pred code (t :: v_runnable));
}
```
...
fn take_one_h11 (#code:vcode)
  (t : task_t code.t)
  (v_runnable : list (task_t code.t))
  requires all_state_pred code (t :: v_runnable)
  ensures  task_thunk_typing code t
        ** state_pred t.pre t.post t.h
        ** all_state_pred code v_runnable
{
  unfold (all_state_pred code (t :: v_runnable));
}
```
...
let extension_preserves_memP (#a:Type) (x:a) (l1 l2:list a)
  : Lemma (requires List.memP x l1 /\ list_extension l1 l2)
          (ensures List.memP x l2)
          [SMTPat (list_extension l1 l2); SMTPat (List.memP x l1)]
  = ()
...
let list_preorder_mono_memP (#a:Type) (x:a) (l1 l2:list a)
  ...
  =
  FStar.ReflexiveTransitiveClosure.stable_on_closure list_extension (List.memP x) ()
...
// FIXME. This is the same impredicativity problem.
// Do tasks pre/posts have to be big? Can they use locks?
assume LockInvIsBig : forall code runnable g_runnable. is_big (lock_inv code runnable g_runnable)
...
let pool_alive (#[exact (`1.0R)] f : perm) (code:vcode) (p:pool code.t) : vprop =
  lock_alive p.lk #(f /. 2.0R) (lock_inv code p.runnable p.g_runnable)

let pool_alive_is_big (f:perm) (code:vcode) (p:pool code.t)
  : Lemma (is_big (pool_alive #f code p)) [SMTPat (pool_alive #f code p)]
  = admit() // axiom, impredicativity

let pool_alive0 (code:vcode) (p:pool code.t) : vprop =
  lock_alive0 p.lk (lock_inv code p.runnable p.g_runnable)
...
fn rec extract_state_pred
  ...
  requires all_state_pred code ts ** pure (List.memP t ts)
  ensures (task_thunk_typing code t ** state_pred t.pre t.post t.h)
       ** trade (task_thunk_typing code t ** state_pred t.pre t.post t.h) (all_state_pred code ts) // trade to put things back together
  decreases ts
{
  open Pulse.Lib.InvList;
  match ts {
    ...
    Cons t' ts' -> {
      ...
      if not b {
        ...
        ghost
        fn aux (_:unit)
          requires (task_thunk_typing code t' ** state_pred t'.pre t'.post t'.h) **
                   all_state_pred code ts'
          ensures  all_state_pred code ts
        {
          add_one_state_pred t' ts';
        };
        intro_trade (all_state_pred code ts') (all_state_pred code ts)
                    (task_thunk_typing code t' ** state_pred t'.pre t'.post t'.h) aux;

        trade_compose (task_thunk_typing code t ** state_pred t.pre t.post t.h) (all_state_pred code ts') (all_state_pred code ts);
      } else {
        rewrite each t' as t;
        take_one_h11 t ts';
        ...
        ghost
        fn aux (_:unit)
          requires (pure (ts == t'::ts') ** all_state_pred code ts')
                ** (task_thunk_typing code t ** state_pred t'.pre t'.post t'.h)
          ensures   all_state_pred code ts
        {
          add_one_state_pred t' ts';
        };
        intro_trade (task_thunk_typing code t ** state_pred t.pre t.post t.h) (all_state_pred code ts)
                    (pure (ts == t'::ts') ** all_state_pred code ts') aux;
    ...
    ensures  pool_alive #pf code p ** joinable p post h
{
  ...
  acquire p.lk;
  unfold (lock_inv code p.runnable p.g_runnable);
  ...
  fold (lock_inv code p.runnable p.g_runnable);
...
let handle_done (h:handle) : boxable =
  exists* (st : task_state).
    pure (st == Done \/ st == Claimed) **
    AR.snapshot h.g_state st

let task_done (#code_t:Type u#2) (t : task_t code_t) : boxable =
  handle_done t.h

let rec all_tasks_done (#code_t:Type u#2) (ts : list (task_t code_t)) : boxable =
  match ts with
  | [] -> emp
  | t::ts' ->
    task_done t **
...
let pool_done (code:vcode) (p : pool code.t) : vprop =
  exists* ts.
    lock_alive0 p.lk (lock_inv code p.runnable p.g_runnable) **
    BAR.pts_to p.g_runnable #0.5R ts ** all_state_pred code ts ** all_tasks_done ts

let pool_done_is_big (code:vcode) (p : pool code.t)
  ...
  ensures  pool_done code p ** code.up post
{
  ...
  with v_runnable. assert (BAR.pts_to p.g_runnable #0.5R v_runnable);
  with t. assert (task_spotted p t);
...
fn __disown (#code:vcode) (#p:pool code.t)
  ...
  ensures  pledge emp_inames (pool_done code p) (code.up post)
{
  make_pledge emp_inames (pool_done code p) (code.up post) (joinable p post h)
...
fn __spawn_ (#code:vcode) (p:pool code.t)
    ...
    (f : unit -> stt unit (code.up pre) (fun _ -> code.up post))
    requires pool_alive #pf code p ** code.up pre
    ensures  pool_alive #pf code p ** pledge emp_inames (pool_done code p) (code.up post)
{
  let h = spawn p f;
...
fn __pool_done_handle_done_aux2 (#code:vcode) (#p:pool code.t)
      ...
      (h : handle)
      (ts : list (task_t code.t))
  requires BAR.pts_to p.g_runnable #0.5R ts ** all_tasks_done ts ** handle_spotted p post h
  ensures  BAR.pts_to p.g_runnable #0.5R ts ** all_tasks_done ts ** handle_done h
{
...
fn __pool_done_handle_done (#code:vcode) (#p:pool code.t)
      ...
      (h : handle)
      (_ : unit)
  requires pool_done code p ** handle_spotted p post h
  ensures  pool_done code p ** handle_done h
{
  unfold (pool_done code p);
...
fn pool_done_handle_done (#code:vcode) (#p:pool code.t)
  ...
  ensures pledge emp_inames (pool_done code p) (handle_done h)
{
  make_pledge emp_inames (pool_done code p) (handle_done h) (handle_spotted p post h)
...
fn weaken_vopt (#a:Type u#2) (o : option a)
    ...
    (f : (x:a) -> stt_ghost unit emp_inames (extra ** p1 x) (fun _ -> p2 x))
...
fn rec grab_work'' (#code:vcode) (p:pool code.t) (v_runnable : list (task_t code.t))
  requires all_state_pred code v_runnable
  returns  topt : option (task_t code.t)
  ensures  all_state_pred code v_runnable
        ** vopt topt (fun t ->
             code.up t.pre ** pts_to t.h.state #0.5R Running ** pure (List.memP t v_runnable) ** task_thunk_typing code t)
{
  match v_runnable {
    Nil -> {
      // intro_vopt_none;
      ...
      rewrite emp
          as vopt topt (fun t -> code.up t.pre ** pts_to t.h.state #0.5R Running ** pure (List.memP t v_runnable) ** task_thunk_typing code t);
      topt
    }
    Cons t ts -> {
      take_one_h11 t ts;
      ...
      match st {
        Ready -> {
          ...
          rewrite code.up t.pre ** pts_to t.h.state #0.5R Running ** pure (List.memP t v_runnable) ** task_thunk_typing code t
               as vopt topt (fun t -> code.up t.pre ** pts_to t.h.state #0.5R Running ** pure (List.memP t v_runnable) ** task_thunk_typing code t);
        ...
        _ -> {
          ...
          ghost fn weaken (t : task_t code.t)
            requires emp ** (code.up t.pre ** pts_to t.h.state #0.5R Running ** pure (List.memP t ts) ** task_thunk_typing code t)
            ensures  code.up t.pre ** pts_to t.h.state #0.5R Running ** pure (List.memP t v_runnable) ** task_thunk_typing code t
          {
            ()
          };
...
fn rec grab_work' (#code:vcode) (p:pool code.t)
  requires lock_inv code p.runnable p.g_runnable
  returns  topt : option (task_t code.t)
  ensures  lock_inv code p.runnable p.g_runnable
        ** vopt topt (fun t ->
             code.up t.pre ** pts_to t.h.state #0.5R Running ** task_spotted p t ** task_thunk_typing code t)
{
  unfold (lock_inv code p.runnable p.g_runnable);
  let v_runnable = Big.op_Bang p.runnable;
  ...
  ghost fn spot (t:task_t code.t)
    requires BAR.snapshot p.g_runnable v_runnable ** (code.up t.pre ** pts_to t.h.state #0.5R Running ** pure (List.memP t v_runnable) ** task_thunk_typing code t)
    ensures  code.up t.pre ** pts_to t.h.state #0.5R Running ** task_spotted p t ** task_thunk_typing code t
  {
...
```pulse
fn grab_work (#code:vcode) (#f:perm) (p:pool code.t)
...
```pulse
fn put_back_result (#code:vcode) (#f:perm) (p:pool code.t) (t : task_t code.t)
  requires pool_alive #f code p **
           task_spotted p t **
           code.up t.post **
  ...
  ensures  pool_alive #f code p
{
  ...
  match topt {
    ...
    Some t -> {
      ...
      (* sigh *)
      rewrite (fun t -> code.up t.pre ** pts_to t.h.state #0.5R Running ** task_spotted p t ** task_thunk_typing code t) t
           as code.up t.pre ** pts_to t.h.state #0.5R Running ** task_spotted p t ** task_thunk_typing code t;
      perf_work t;
      put_back_result p t;
      worker p
    }
...
fn rec check_if_all_done
  (#code:vcode)
  (ts : list (task_t code.t))
  requires all_state_pred code ts
  returns  b : bool
  ensures  all_state_pred code ts ** ite b (all_tasks_done ts) emp
{
  match ts {
    Nil -> {
...
fn __share_alive
  ...
  (f:perm)
  requires pool_alive #f code p
  ensures  pool_alive #(f /. 2.0R) code p ** pool_alive #(f /. 2.0R) code p
{
  unfold (pool_alive #f code p);
  Pulse.Lib.SpinLock.share #_ #(f /. 2.0R) p.lk;
  fold (pool_alive #(f /. 2.0R) code p);
  fold (pool_alive #(f /. 2.0R) code p);
}
```
...
fn __gather_alive
  ...
  (p : pool code.t)
  (f:perm)
  requires pool_alive #(f /. 2.0R) code p ** pool_alive #(f /. 2.0R) code p
  ensures  pool_alive #f code p
{
  unfold (pool_alive #(f /. 2.0R) code p);
  unfold (pool_alive #(f /. 2.0R) code p);
  Pulse.Lib.SpinLock.gather p.lk;
  
  rewrite lock_alive p.lk #(f /. 2.0R /. 2.0R +. f /. 2.0R /. 2.0R) (lock_inv code p.runnable p.g_runnable)
       as lock_alive p.lk #(f /. 2.0R) (lock_inv code p.runnable p.g_runnable);
  ...
  ensures pool_alive code p
{
  ...
  fold (lock_inv code runnable g_runnable);
  let lk = new_lock (lock_inv code runnable g_runnable);
  ...
  Pulse.Lib.SpinLock.share #_ #1.0R p.lk;
  fold (pool_alive code p);
  fold (pool_alive code p);
...
```pulse
ghost
fn __pool_alive0_inj_l (code1 code2 : vcode) (p : pool code1.t)
  (#_:squash (code1.t == code2.t))
  (f1 : perm)
  requires pool_alive #f1 code1 p ** pool_alive0 code2 p
  ensures  pool_alive #f1 code1 p ** pool_alive0 code1 p
{
  unfold (pool_alive #f1 code1 p);
  unfold (pool_alive0 code2 p);

  assert (lock_alive p.lk #(f1 /. 2.0R) (lock_inv code1 p.runnable p.g_runnable));

  (* Match the parameters of the projectors. *)
  rewrite
    (lock_alive0 (Mkpool_st?.lk #code2.t p)
                 (lock_inv code2 (Mkpool_st?.runnable #code2.t p) (Mkpool_st?.g_runnable #code2.t p)))
  as
    (lock_alive0 (Mkpool_st?.lk #code1.t p)
                 (lock_inv code2 (Mkpool_st?.runnable #code1.t p) (Mkpool_st?.g_runnable #code1.t p)))
  ;

  assert (lock_alive0 p.lk (lock_inv code2 p.runnable p.g_runnable));

  Pulse.Lib.SpinLock.lock_alive0_inj_l
    p.lk
        #(f1 /. 2.0R)
        #(lock_inv code1 p.runnable p.g_runnable)
        #(lock_inv code2 p.runnable p.g_runnable)
    ;

  assert (lock_alive p.lk #(f1 /. 2.0R) (lock_inv code1 p.runnable p.g_runnable)
       ** lock_alive0 p.lk (lock_inv code1 p.runnable p.g_runnable));

  fold (pool_alive #f1 code1 p);
  fold (pool_alive0 code1 p);
}
```

```pulse
ghost
fn __pool_alive0_inj_r (code1 code2 : vcode) (p : pool code1.t)
  (#_:squash (code1.t == code2.t))
  (f1 : perm)
  requires pool_alive #f1 code1 p ** pool_alive0 code2 p
  ensures  pool_alive #f1 code2 p ** pool_alive0 code2 p
{
  unfold (pool_alive #f1 code1 p);
  unfold (pool_alive0 code2 p);
  ...
  rewrite
    (lock_alive (Mkpool_st?.lk #code1.t p)
                #(f1 /. 2.0R)
                (lock_inv code1 (Mkpool_st?.runnable #code1.t p) (Mkpool_st?.g_runnable #code1.t p)))
  as
    (lock_alive (Mkpool_st?.lk #code2.t p)
                #(f1 /. 2.0R)
                (lock_inv code1 (Mkpool_st?.runnable #code2.t p) (Mkpool_st?.g_runnable #code2.t p)))
  ;
  rewrite
    (lock_alive0 (Mkpool_st?.lk #code2.t p)
                 (lock_inv code2 (Mkpool_st?.runnable #code2.t p) (Mkpool_st?.g_runnable #code2.t p)))
  ...
  Pulse.Lib.SpinLock.lock_alive0_inj_r
    (Mkpool_st?.lk #code2.t p)
        #(f1 /. 2.0R)
        #(lock_inv code1 (Mkpool_st?.runnable #code2.t p) (Mkpool_st?.g_runnable #code2.t p))
        #(lock_inv code2 (Mkpool_st?.runnable #code2.t p) (Mkpool_st?.g_runnable #code2.t p))
    ;

  fold (pool_alive #f1 code2 p);
  fold (pool_alive0 code2 p);
}
```
...
```pulse
ghost
fn __get_pool_alive0 (#code : vcode) (p : pool code.t)
  (#f : perm)
  requires pool_alive #f code p
  ensures  pool_alive #f code p ** pool_alive0 code p
{
  unfold (pool_alive #f code p);
  get_lock_alive0 p.lk;
  fold (pool_alive #f code p);
  fold (pool_alive0 code p);
}
```

```pulse
ghost
fn __pool_alive_inj_l (code1 code2 : vcode) (p : pool code1.t)
  (#_:squash (code1.t == code2.t))
  (f1 f2 : perm)
  requires pool_alive #f1 code1 p ** pool_alive #f2 code2 p
  ensures  pool_alive #f1 code1 p ** pool_alive #f2 code1 p
{
  __get_pool_alive0 #code1 p;
  __pool_alive0_inj_r code2 code1 p f2;
  drop_ (pool_alive0 code1 p);
}
```

```pulse
ghost
fn __pool_alive_inj_r (code1 code2 : vcode) (p : pool code1.t)
  (#_:squash (code1.t == code2.t))
  (f1 f2 : perm)
  requires pool_alive #f1 code1 p ** pool_alive #f2 code2 p
  ensures  pool_alive #f1 code2 p ** pool_alive #f2 code2 p
{
  __get_pool_alive0 #code2 p;
  __pool_alive0_inj_r code1 code2 p f1;
  drop_ (pool_alive0 code2 p);
}
```

```pulse
ghost
fn __pool_done_inj (code1 code2 : vcode) (p : pool code1.t)
  (#_:squash (code1.t == code2.t))
  (f1 : perm)
  requires pool_alive #f1 code1 p ** pool_done code2 p
  ensures  pool_alive #f1 code1 p ** pool_done code1 p
{
  unfold (pool_alive #f1 code1 p);
  unfold (pool_done  code2 p);
  with ts. assert (BAR.pts_to (Mkpool_st?.g_runnable #code2.t p) #0.5R ts);
  assert (all_state_pred code2 ts);

  assert (lock_alive p.lk #(f1 /. 2.0R) (lock_inv code1 p.runnable p.g_runnable));

  rewrite
    (lock_alive0 (Mkpool_st?.lk #code2.t p) (lock_inv code2 (Mkpool_st?.runnable #code2.t p) (Mkpool_st?.g_runnable #code2.t p)))
  as
    (lock_alive0 (Mkpool_st?.lk #code1.t p) (lock_inv code2 (Mkpool_st?.runnable #code1.t p) (Mkpool_st?.g_runnable #code1.t p)))
  ;
  ...
  rewrite
    (BAR.pts_to (Mkpool_st?.g_runnable #code2.t p) #0.5R ts)
  as
    (BAR.pts_to (Mkpool_st?.g_runnable #code1.t p) #0.5R ts);

  (* FIXME: a problem here: even if we can change the lock_alive0
  to the code we want, we cannot change the all_state_pred code predicates
  between them. Even if the spinlock module provided an equality between
  the lock_inv predicates, lock_inv is not necesarilly injective in the
  code (since parts of it may be unused; and in the best case, it's only
  extensionally equal).  *)
  drop_ (all_state_pred code2 ts);
  assume_ (all_state_pred code1 ts);
  // rewrite (all_state_pred code2 ts)
  //      as (all_state_pred code1 ts);

  (* This one is fine, it's just about the type index. *)
  rewrite (all_tasks_done #code2.t ts)
       as (all_tasks_done #code1.t ts);

  fold (pool_alive #f1 code1 p);
  fold (pool_done code1 p);
}
```

```pulse
ghost
fn __pool_done0_inj_r (code1 code2 : vcode) (p : pool code1.t)
  (#_:squash (code1.t == code2.t))
  requires pool_done code1 p ** pool_alive0 code2 p
  ensures  pool_done code2 p ** pool_alive0 code2 p
{
  (* I think this may be actually implementable by just duplicating the lock_alive0
  instead of using the inj_both lemma. But it anyway has the unrelated FIXME. *)
  unfold (pool_done code1 p);
  unfold (pool_alive0 code2 p);
  with ts. assert (BAR.pts_to (Mkpool_st?.g_runnable #code1.t p) #0.5R ts);
  assert (all_state_pred code1 ts);

  assert (lock_alive0 p.lk (lock_inv code1 p.runnable p.g_runnable));
  ...
  Pulse.Lib.SpinLock.lock_alive0_inj_both
    p.lk
        #(lock_inv code1 p.runnable p.g_runnable)
        #(lock_inv code2 p.runnable p.g_runnable)
    ;
  ...
  assert (lock_alive0 p.lk (lock_inv code2 p.runnable p.g_runnable)
       ** lock_alive0 p.lk (lock_inv code2 p.runnable p.g_runnable));
  ...
  code (since parts of it may be unused; and in the best case, it's only
  extensionally equal).  *)
  drop_ (all_state_pred code1 ts);
  assume_ (all_state_pred code2 ts);
  // rewrite (all_state_pred code2 ts)
  //      as (all_state_pred code1 ts);

  (* This one is fine, it's just about the type index. *)
  rewrite (all_tasks_done #code1.t ts)
       as (all_tasks_done #code2.t ts);

  (* Matching projectors, should really be automatic. *)
  rewrite each code1.t as code2.t;

  fold (pool_alive0 code2 p);
  fold (pool_done code2 p);
}
```
...
let pool_alive_inj_l = __pool_alive_inj_l
let pool_alive_inj_r = __pool_alive_inj_r
let pool_done_inj = __pool_done_inj
let get_pool_alive0 = __get_pool_alive0
let pool_alive0_inj_l = __pool_alive0_inj_l
let pool_alive0_inj_r = __pool_alive0_inj_r
let pool_done0_inj_r = __pool_done0_inj_r
